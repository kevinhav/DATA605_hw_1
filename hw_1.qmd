...
We can now view the animation in its entirely - a few still frames for PDF format below.
...
Considering the transpose $A^T=\begin{bmatrix}a&c\\b&d\end{bmatrix}$, we can see that the determinant is non-negative given the commutative property of *scalar* multiplication as the term $bc$ and $cb$ are the same; $det|A^T| = A^{T}_{11}A^{T}_{22} - A^{T}_{21}A^{T}_{12} = ad - cb$.
...
If the matrix is non-zero, the smallest it could possibly be is $1x1$. Therefore, minimum rank must also be $r_{min}=1$.
...
As a square matrix, $A$ is considered diagonalizable if it is similar to a diagonal matrix. We would like to find some non-singular matrix $S$ that allows us to diagonlize $A$ such that $A_{diag} = S^{-1}A{S}$.
